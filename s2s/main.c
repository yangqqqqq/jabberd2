    ...
    if(s2s->check_queue > 0) {
        if(xhash_iter_first(s2s->outq))
            do {
                ...
                c = (char*)memchr(rkey, '/', keylen);
                c++;
                c_len = keylen - (c - rkey);

                /* dns lookup timeout check first */
				dns = (dnscache_t)xhash_getx(s2s->dnscache, c, c_len);
                ...
                /* get the conn */
                conn = (conn_t)xhash_getx(s2s->out_dest, c, c_len);
                if(conn == NULL) {
                    if(jqueue_size(q) > 0) {
                ...
                if(!conn->online && now > conn->init_time + s2s->check_queue) {
                    ...
                    if (s2s->dns_bad_timeout > 0) {
                        ...
                        bad = (dnsres_t)xhash_get(s2s->dns_bad, ipport);
                        if (bad == NULL) {
...
/* get winsock running */
    ...
    /* cmdline parsing */
    while((optchar = getopt(argc, argv, (const char*)"Dc:hi:?")) >= 0)
    {
        switch(optchar)
        {
            case 'c':
